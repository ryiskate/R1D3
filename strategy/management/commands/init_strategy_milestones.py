...
class Command(BaseCommand):
    help = 'Initialize strategy milestones data'

    def handle(self, *args, **options):
        # Define the milestones data
        milestones_data = {
            "1": [  # Phase 1: Indie Game Development
                {
                    'id': 1001,
            ...
            "2": [  # Phase 2: Arcade Machine Development
                {
                    'id': 2001,
            ...
            "3": [  # Phase 3: Theme Park Attractions
                {
                    'id': 3001,
        ...
        file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'fixtures', 'initial_milestones.json')
        
        # Create fixtures directory if it doesn't exist
        os.makedirs(os.path.dirname(file_path), exist_ok=True)
        
        with open(file_path, 'w') as f:
            json.dump(milestones_data, f, indent=4)
        
        self.stdout.write(self.style.SUCCESS(f'Successfully created initial milestones data at {file_path}'))
